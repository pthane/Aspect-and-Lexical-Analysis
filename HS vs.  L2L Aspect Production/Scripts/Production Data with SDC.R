...
SDC <- read_csv("./CSV Files/Comparison/Comparison EPT Preterit Data.csv")
Master_Aggregate <- rbind(HS, L2, SDC)
...
plot_model(Aggregate_Model, show.values = TRUE, show.intercept = TRUE, value.offset = .3, transform = NULL, vline.color = "black") +
  scale_x_discrete(labels = c("L2L : Proficiency : LF", "HS : Proficiency : LF", "Proficiency : LF", "L2L : LF", "HS : LF", "L2L : Proficiency", "HS : Proficiency", "Lexical Frequency (LF)", "Proficiency", "L2L Group", "HS Group", "(Intercept)")) +
  labs(title = "Results of Three-Group Model", y = "Beta Estimates") +
