...
Aggregate_Data %>%
  ggplot(aes(x = MainVerb, y = Response, shape = NewTask, color = ExpGroup)) + 
  ...
  labs(x = "State verb (from most to least frequent)", y = "Proportion of preterit responses",
       shape = "Task", title = "Average Use of Preterit by Verb and Group") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))


## Regularity and task
Aggregate_Data %>%
  ggplot(aes(x = Reg_Main, y = Response, color = NewTask)) + 
...
Verb_Avg_Master %>%
  ...
  labs(x = "Log-transformed lexical frequency in Davies (2016)", y = "Proportion of preterit responses", color = "Task", 
       title = "Preterit Use by Lexical Frequency and Task") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
Preterit_Proficiency %>% 
  ...
  scale_y_continuous(breaks = seq (0, 1, 0.2),
                     limits = c(0, 1)) +
  labs(x = "DELE proficiency score", y = "Proportion of preterit responses by participant", color = "Token Frequency", 
       title = "HS' Proportion of Preterit by Task and Proficiency") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
Preterit_FofA %>% 
  ...
  scale_y_continuous(breaks = seq (0, 1, 0.2),
                     limits = c(0, 1)) +
  labs(x = "Self-reported frequency of use", y = "Proportion of preterit responses by participant", color = "Token Frequency", 
       title = "HS' Proportion of Preterit by Task and Frequency of Use") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
