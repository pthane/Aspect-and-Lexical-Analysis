...
Lexical_Item_Report = read.csv("./CSV Files/Lexical Item Analysis/Lexical Item Report.csv") %>% 
  mutate(Davies_Log = log(Davies_Rating))
...
## CORPES-SRLF graphs
Average_by_LI %>% 
  ggplot(., aes(x = log(CORPES_Rating), y = SRLF_Avg)) + 
  geom_point() + 
  geom_smooth(method = lm) +
  scale_y_continuous(breaks = seq (0, 9, 2),
                     limits = c(0, 9)) +
  labs(x = "Log-transformed lemma frequency in CORPES XXI", y = "Average of verb ratings across participants", title = "Correlation between CORPES XXI and self-ratings") +
  theme(axis.title = element_text(face = "bold"),
        plot.title = element_text(hjust = 0.5, face = "bold"))


# Lexical item histogram
hist(Lexical_Item_Report$Davies_Log)